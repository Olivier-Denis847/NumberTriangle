...
public class NumberTriangle {
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        int width = 0;
        NumberTriangle[] items = new NumberTriangle[width];
        ...
        while (line != null) {
            ...
            NumberTriangle[] row = new NumberTriangle[width + 1];
            for (int i = 0; i < width + 1; i++)
                row[i] = new  NumberTriangle(Integer.parseInt(values[i]));
            if (width == 0)
                top = row[0];
            else {
                for (int i = 0; i < width; i++) {
                    items[i].setLeft(row[i]);
                    items[i].setRight(row[i+1]);
                }
            }
            width++;
            items = row;
